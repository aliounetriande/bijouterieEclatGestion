...
function buildOrderPdf(order, type) {
  ...
  const lines = [
    ...
    'BT /F2 18 Tf 63 746 Td (BE) Tj ET',
    '0 0 0 rg',

    // ── Nom + adresse ──
    'BT /F2 16 Tf 120 780 Td (BIJOUTERIE ECLAT) Tj ET',
    '0.3 0.3 0.3 rg',
    'BT /F1 8 Tf 120 766 Td (Sis en face de la porte 14 du Stade Municipal) Tj ET',
    'BT /F1 8 Tf 120 756 Td (a 50m de l\'immeuble Photo Rapide) Tj ET',
    'BT /F1 8 Tf 120 744 Td (Tel.: 71 83 21 26 / 78 49 18 11 / 55 45 70 71) Tj ET',
    '0 0 0 rg',

    // ── Type de document + référence ──
    `BT /F2 12 Tf 430 780 Td (${title}) Tj ET`,
    '0.3 0.3 0.3 rg',
    `BT /F1 10 Tf 430 765 Td (${escapePdfText(order.order_id)}) Tj ET`,
    '0 0 0 rg',

    // ── Ligne séparatrice ──
    '0.8 0.8 0.8 RG', '0.5 w', '50 725 m 545 725 l S', '0 0 0 RG',

    // ── Infos client ──
    'BT /F2 11 Tf 50 700 Td (Nom :) Tj ET',
    `BT /F1 11 Tf 130 700 Td (${escapePdfText(order.customer)}) Tj ET`,
    'BT /F2 11 Tf 50 683 Td (Tel :) Tj ET',
    `BT /F1 11 Tf 130 683 Td (${escapePdfText(order.phone || 'Non renseigne')}) Tj ET`,
    'BT /F2 11 Tf 300 700 Td (Date commande :) Tj ET',
    `BT /F1 11 Tf 410 700 Td (${escapePdfText(order.date_created)}) Tj ET`,
  ];

  if (isReceipt && order.date_delivered) {
    lines.push('BT /F2 11 Tf 300 683 Td (Date livraison :) Tj ET');
    lines.push(`BT /F1 11 Tf 410 683 Td (${escapePdfText(order.date_delivered)}) Tj ET`);
  }

  lines.push('0.8 0.8 0.8 RG', '0.5 w', '50 668 m 545 668 l S', '0 0 0 RG');

  // Détails bijou
  lines.push('BT /F2 13 Tf 50 645 Td (Bijou commande) Tj ET');
  lines.push(`BT /F1 11 Tf 50 625 Td (${escapePdfText(order.item_name)}) Tj ET`);
  if (order.item_description) {
    lines.push('0.4 0.4 0.4 rg');
    lines.push(`BT /F1 10 Tf 50 608 Td (${escapePdfText(order.item_description)}) Tj ET`);
    lines.push('0 0 0 rg');
  }

  lines.push('0.8 0.8 0.8 RG', '0.5 w', '50 590 m 545 590 l S', '0 0 0 RG');

  // Montants
  lines.push('BT /F2 11 Tf 50 565 Td (Prix total :) Tj ET');
  lines.push(`BT /F1 11 Tf 400 565 Td (${formatPricePdf(order.price)}) Tj ET`);
  lines.push('BT /F2 11 Tf 50 545 Td (Avance versee :) Tj ET');
  lines.push(`BT /F1 11 Tf 400 545 Td (${formatPricePdf(order.advance)}) Tj ET`);

  lines.push('0.8 0.8 0.8 RG', '0.5 w', '50 533 m 545 533 l S', '0 0 0 RG');

  if (isReceipt) {
    lines.push('BT /F2 13 Tf 50 510 Td (SOLDE PAYE) Tj ET');
    lines.push(`BT /F2 13 Tf 400 510 Td (${formatPricePdf(remaining)}) Tj ET`);
    lines.push('0.8 0.8 0.8 RG', '0.5 w', '50 497 m 545 497 l S', '0 0 0 RG');
    lines.push('BT /F2 14 Tf 50 475 Td (TOTAL PAYE) Tj ET');
    lines.push(`BT /F2 14 Tf 400 475 Td (${formatPricePdf(order.price)}) Tj ET`);
  } else {
    lines.push('0.788 0.655 0.353 rg');
    lines.push('BT /F2 13 Tf 50 510 Td (RESTE A PAYER) Tj ET');
    lines.push(`BT /F2 13 Tf 400 510 Td (${formatPricePdf(remaining)}) Tj ET`);
    lines.push('0 0 0 rg');
  }
...
function buildPdf(sale) {
  const lines = [
    ...
    // ── Type de document + référence ──
    'BT /F2 12 Tf 460 780 Td (Recu) Tj ET',
    '0.3 0.3 0.3 rg',
    `BT /F1 10 Tf 460 765 Td (${escapePdfText(sale.id)}) Tj ET`,
    '0 0 0 rg',
    ...
    // ── Infos client ──
    'BT /F2 11 Tf 50 700 Td (Nom :) Tj ET',
    `BT /F1 11 Tf 110 700 Td (${escapePdfText(sale.customer)}) Tj ET`,
    'BT /F2 11 Tf 50 683 Td (Tel :) Tj ET',
    `BT /F1 11 Tf 110 683 Td (${escapePdfText(sale.phone || 'Non renseigne')}) Tj ET`,
    'BT /F2 11 Tf 350 700 Td (Date :) Tj ET',
    `BT /F1 11 Tf 400 700 Td (${escapePdfText(sale.date)}) Tj ET`,

    // ── Ligne séparatrice ──
    '0.8 0.8 0.8 RG', '0.5 w', '50 668 m 545 668 l S', '0 0 0 RG',

    // ── En-tête tableau ──
    '0.4 0.4 0.4 rg',
    'BT /F2 10 Tf 50 650 Td (Produit) Tj ET',
    'BT /F2 10 Tf 300 650 Td (Qte) Tj ET',
    'BT /F2 10 Tf 420 650 Td (Prix) Tj ET',
    '0 0 0 rg',
  ];
